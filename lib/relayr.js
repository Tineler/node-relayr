...
Relayr.prototype.reconnect = function(token, dev_id){
    var mqtt = this.mqtt;
    log.debug("reconnecting");

    // remove the remaining channels
    api.channels(token, dev_id, function(channels) {
        for (var channel in channels) {
            api.deleteChannel(token, channel, function() {
                log.info("channel " + channel +  " deleted");
            });
        }
    })
